...
int process_command(struct command_t *command) {
  ...
  if (pid == 0) // child
  {
    ...
    while (temp_path != NULL) {
      strcpy(full_path, temp_path); // copies part of already parsed path to full path
      strcat(full_path, "/"); // appends / char
      strcat(full_path, command->name); // appends command name to full path

      execv(full_path, command->args); // executes path with arguments
